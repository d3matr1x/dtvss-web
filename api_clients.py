...
# Canonical name map — defined once, not per-loop-iteration
# Maps raw FDA firm name fragments → clean display names
CANONICAL_NAMES = {
    "medtronic": "Medtronic",
    "abbott": "Abbott",
    "biotronik": "Biotronik",
...
_SUFFIX_PATTERN = _re_module.compile(
    ...
    for pc in CONNECTED_PRODUCT_CODES:
        try:
            ...
            for result in data.get("results", []):
                ...
                # Map raw FDA name to canonical display name
                firm_lower = firm_name.lower().strip()
                cleanmatch = _SUFFIX_PATTERN.sub('', firm_lower).strip(",. ")
...
def build_manufacturer_search_queries(manufacturer_name: str) -> list[str]:
    ...
    Flow: manufacturer name → NVD CPE keyword search → discover products → search CVEs.
    First user triggers CPE lookup (2-3 sec), cached 24hr after that.
    """
    key = manufacturer_name.lower().strip()
    products = refresh_cpe_product_map(key)
    product_list = products.get(key, [])

    queries = [manufacturer_name]

    for product in product_list:
        if product not in queries:
            queries.append(product)

    # If CPE returned nothing, add known alternate names
    if not product_list:
        for alt in MANUFACTURER_ALIASES.get(key, []):
...
# ═══════════════════════════════════════════════════════════════════════
# NVD CPE Product Map — Keyword search, no hardcoded vendor strings
# ═══════════════════════════════════════════════════════════════════════
...
# Alternate search names for manufacturers whose canonical dropdown name
# differs from how NVD/CPE references them.
# Source: validated against NVD CVE descriptions in the 96-CVE calibration dataset
# and CISA ICS-CERT medical advisory history (MedCrypt 2024 ICS-CERT analysis).
# These are NOT guesses — each alias has confirmed CVE matches in NVD.
MANUFACTURER_ALIASES = {
    "bd (becton dickinson)": ["Becton Dickinson", "BD Alaris", "Alaris"],  # NVD uses full name, not "BD"
    "dräger": ["Draeger", "Drager"],  # NVD uses ASCII spelling without umlaut
    "tandem diabetes": ["Tandem Diabetes Care"],  # NVD uses full company name
    "icu medical": ["Hospira"],  # ICU Medical acquired Hospira; legacy CVEs under old name
    "abbott": ["St. Jude Medical"],  # Abbott acquired St. Jude; cardiac CVEs under old name
    "fresenius kabi": ["Fresenius"],  # NVD sometimes uses parent company name only
    "b. braun": ["B Braun"],  # NVD inconsistent on period in "B. Braun" vs "B Braun"
}


def refresh_cpe_product_map(manufacturer_key: str) -> dict:
    """
    Query NVD CPE API with keyword search to discover all products
    registered under a manufacturer. No hardcoded CPE vendor strings needed.
    Lazy-cached per manufacturer for 24 hours.
    """
    import time as _time
    now = _time.time()

    cached = _cpe_product_cache["by_manufacturer"].get(manufacturer_key)
    cached_at = _cpe_product_cache["per_mdm_fetched"].get(manufacturer_key, 0)

    if cached is not None and (now - cached_at) < CPE_CACHE_TTL:
        return {manufacturer_key: cached}

    products = set()

    # Build search terms: canonical name + any aliases
    search_terms = [manufacturer_key]
    for alias in MANUFACTURER_ALIASES.get(manufacturer_key, []):
        ...
        try:
            ...
            for entry in data.get("products", []):
                ...
                if len(parts) >= 5:
                    ...
                    # Skip if product is wildcard or too short
                    product_clean = product_raw.replace("_", " ").strip()
                    if not product_clean or len(product_clean) < 3 or product_clean == "*":
                        continue

                    vendor_clean = vendor_raw.replace("_", " ").strip()

                    # Store: "vendor product" (scoped search) and "product" alone
                    products.add(f"{vendor_clean} {product_clean}")

                    # Also add standalone product name if it's specific enough
                    # Skip single common words that would return noise
                    generic = {"server", "client", "firmware", "software", "system",
                               "monitor", "controller", "device", "manager", "gateway",
                               "agent", "cloud", "web", "app", "mobile", "desktop"}
                    if product_clean.lower() not in generic and len(product_clean) >= 4:
                        products.add(product_clean)

        except Exception as e:
            print(f"CPE lookup failed for '{term}': {e}")

        _time.sleep(0.8)

    product_list = sorted(list(products), key=len, reverse=True)

    _cpe_product_cache["by_manufacturer"][manufacturer_key] = product_list
    _cpe_product_cache["per_mdm_fetched"][manufacturer_key] = now

    return {manufacturer_key: product_list}
